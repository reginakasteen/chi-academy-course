...
export default function NewPostForm({ onSubmit }: NewPostFormProps) {
  ...
  return (
    ...
    >
      {({ values, setFieldValue, handleChange, handleBlur, errors, touched, isSubmitting, resetForm }) => (
        <Form>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2, maxWidth: 400, mx: "auto", mt: 4 }}>
            <Typography variant="h5" textAlign="center">Create New Post</Typography>
            ...
            <Box sx={{ display: "flex", gap: 2, mt: 2 }}>
              ...
              <Button type="button" variant="outlined" color="secondary" onClick={() => { resetForm(); setPreview(null); }} fullWidth>
                Reset
              </Button>
            </Box>
          </Box>
